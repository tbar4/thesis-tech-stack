...
```admonish read-along title="The from-scratch verifier and reward: [BRM] chapters 3, 6.6, 7.6"
For the from-scratch construction of the exact object this chapter reuses, read [BRM] (Raschka, *Build a Reasoning Model From Scratch*) chapter 3, "Evaluating reasoning models," where he builds a programmatic math verifier the same way I built the SDA scorer: a text-generation wrapper produces the completion (3.3), then the pipeline extracts the final answer out of its boxed span (3.4), normalizes the extracted string (3.5), verifies mathematical equivalence rather than string-matching (3.6), and grades the result (3.7), with no judge model anywhere in the loop. That extract-then-normalize-then-verify-then-grade pipeline is precisely the verifier my `rewards.py` imports and wraps. Where I point at this book's chapter 3.4 and move on, [BRM] writes the verifier out line by line, so the two treatments are complementary halves of one object: I frame it as an instrument and reuse it as a reward, and [BRM] shows you how to build the instrument.

Then read [BRM] chapter 6.6, "Calculating rewards," which is the scorer-as-reward identity made literal in code: the same verifier that measured the model in [BRM] chapter 3 becomes the reward term inside the GRPO loop, exactly the substitution this chapter is about. And read [BRM] chapter 7.6, "Adding an explicit format reward," where a separate term pays the policy for emitting the `<think>` structure alongside correctness, which is the same two-component design I argued for above (a dense format scaffold kept well below a sparse correctness objective) and the same separation that the next chapter's reward-hacking discussion (7.4) leans on. Read [BRM] chapters 3, 6.6, and 7.6 together as the from-scratch build of the verifier and the reward wiring that I frame here and reuse throughout Part VII.
```
