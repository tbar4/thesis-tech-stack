...
```admonish read-along title="Go deeper: [BRM] ch. 3, ch. 7.2-7.3, appendix F"
[BRM] is the book to read alongside this chapter, because it builds the exact machinery that produces the numbers I analyze here and then, tellingly, stops one step short of the analysis. Its chapter 3 is a full evaluation harness for a reasoning model: the math verifier and grading logic (3.1 through 3.7), loading the evaluation dataset (3.8), and running the model over it to score every item (3.9). That is precisely the pipeline that emits the per-item, per-sample correct/incorrect logs my `align`, `build`, and `pass_at_k` functions consume; if you want to see how the `EvalLog` records are generated rather than just how they are read, chapter 3 is the answer, and appendix F's survey of evaluation methods is the wider map those choices sit inside. Its chapter 7.2 and 7.3 give the complementary during-training view: tracking GRPO's reward, advantage, and entropy across a run, the before/after picture as the metric curve moves under optimization. Between them, [BRM] covers how to produce a score and how to watch a metric climb.

The honest difference, and it is exactly this chapter's contribution, is what happens after the harness returns. [BRM] reports the evaluation: an accuracy number, a metric curve over training steps. I take those same per-item scores and refuse to report a subtraction of two accuracies, because two accuracies are not a delta a committee can defend. Everything in this chapter that [BRM] does not do lives in that gap: pairing on the shared frozen-suite items so item difficulty cancels (equation 6.2), the paired-bootstrap confidence interval, the sign-flip permutation test and McNemar's test on discordant pairs, Cohen's $d_z$ as a standardized effect size, and the $do$-intervention causal reading that chapter 4.5 hardens with placebo controls. Read [BRM] chapter 3 for how to generate the scores and chapter 7.2-7.3 for how to watch them during the run; read this chapter for how to turn those scores into a claim that survives the three questions a committee actually asks. The harness gives me numbers; the paired statistics are what make the numbers mean something.
```
