...
```admonish read-along title="The same loop with nothing hidden: [BRM] ch. 6 and 7"
Everything in this chapter runs through TRL's `GRPOTrainer`, which is the right tool for fitting a real run into 16GB but is also a closed box: rollouts, per-token log-probs, and the clipped-plus-KL loss all happen inside `trainer.train()`, where I never watch them go by. [BRM] ch. 6 is the companion that opens that box, building the identical loop from scratch in explicit PyTorch with no framework. Read it in the order the trainer executes: sampling the group of rollouts (6.5), scoring them with reward functions (6.6), turning rewards into group-relative advantages (6.7, the same centering-and-scaling formula from the top of this chapter), gathering the sequence log-probs the importance ratio needs (6.8), assembling the GRPO loss (6.9), folding all of it into one GRPO function (6.10) and a training loop (6.11), and loading and evaluating checkpoints (6.12). Once you have seen the loop run by hand, the `GRPOConfig` fields above stop being magic: `num_generations` is the group sampled in 6.5, `beta` and `epsilon` are terms in the 6.9 loss, and `num_iterations` is how many times 6.11 reuses one batch of rollouts before generating fresh ones.

The metrics half of that book maps just as directly onto the curves I told you to watch in the MLflow UI. [BRM] ch. 7.2 executes and inspects a GRPO run, and 7.3 tracks advantage and entropy and plots them, which is exactly the reward-advantage-entropy triple to keep open during a run here: reward tells you the policy is learning, advantage tells you the group still has the spread the update feeds on, and entropy collapsing toward zero is the early warning that the group has gone uniform and the advantage signal is about to vanish. And because a GRPO step is generation-bound (the roofline argument above), the throughput discipline that makes sampling $G$ rollouts per prompt affordable on one card is worth reading from the same source: [BRM] ch. 2 builds the KV cache (2.8) and PyTorch compile (2.9), and appendix E covers batching and throughput. That is the from-scratch view of exactly what Unsloth's in-process vLLM engine does for us in 7.1, and of why `fast_inference` is the difference between decoding a group serially near the 464 tok/s ceiling and keeping the whole group in flight. TRL and Unsloth hide all of this so a run fits; [BRM] shows it so the budget and the config fields make sense.
```
