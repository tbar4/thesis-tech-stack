...
### Building the menu from scratch

The methods above I have *derived and benchmarked*; it is worth pointing at where a reader can *build* them line by line, because doing so makes the token costs this chapter keeps insisting on completely concrete. **[BRM]** ch. 4 constructs the sampling-and-voting half of the menu from nothing: chain-of-thought prompting (4.3), then the two decoding knobs that turn a deterministic model into a sampler, temperature scaling (4.4) and top-$p$ sampling (4.5), and finally self-consistency itself (4.6). Those two knobs are not incidental. Self-consistency and best-of-n both assume I can draw $k$ *different* traces from the same prompt, and temperature plus top-$p$ are exactly the mechanism that makes the samples diverse enough to disagree, which is why my lab sets `temperature=0.8, top_p=0.95` rather than sampling greedily; building the sampler from scratch is the fastest way to see why a too-low temperature collapses the vote to a single repeated answer and a too-high one scatters it into noise. Read [BRM] ch. 4 as the code behind equation (6.6.1) and the self-consistency curve of the lab.

**[BRM]** ch. 5 builds the sequential half, self-refinement, and it is worth reading for a specific contrast with my best-of-n. It starts from a rule-based score (5.3), then develops a token-probability and then a log-probability confidence score (5.4 through 5.6), and closes the self-refinement loop that scores a response and revises it (5.7 through 5.8). The confidence score is the interesting part: it is *verifier-free*, the model grading its own outputs by how confident it was, with no external checker and no ground-truth answer. That places it precisely on the trust-versus-availability curve I drew for verifier types. It is cheaper and more available than my outcome verifier (it needs no gold answer, so it works at deployment where an outcome verifier cannot), and it is correspondingly weaker and hackable, a self-report rather than a check, closer in spirit to self-consistency's popularity vote than to a real verifier. My best-of-n in the lab deliberately uses the outcome verifier instead, precisely because I want the near-ceiling that a real check buys; [BRM] ch. 5's log-probability score is the honest cheaper-but-weaker alternative, and the point of my verifier taxonomy is to let a reader place it and know what they are giving up.

The reason I send the reader to build these rather than just cite them is the matched-budget discipline. Once you have written the self-consistency loop, you can *see* that it costs $k$ forward generations; once you have written the refinement loop, you can see it costs one generation per round plus the scoring passes. The token budget stops being an abstraction the moment it is a `for` loop you wrote, which is exactly the accounting equation (6.6.2) and the compute-per-answer frontier below turn into the one rule that keeps this book's comparisons honest.
