...
### The evaluation method underneath the arms

Before a delta can be attributed to a cause, each arm's output has to be measured, and the choice of how to measure is itself a decision with consequences for the causal claim. [GADP] Lakshmanan & Hapke lay out the menu I am choosing from: an exact or programmatic metric where the answer is checkable, an LLM-as-judge (their Pattern 17, chapter 6) where it is subjective, and path or trajectory evaluation (chapter 5) for agentic systems where the steps matter as much as the endpoint. Their "choosing an evaluation method" is the same call I am making, with one constraint the practitioner framing does not carry: a measurement that is itself noisy or biased does not merely add error, it becomes a confounder in the contrast, so the eval method is part of the causal design rather than a downstream detail.

That constraint is why the arms are scored by the physics oracle (chapter 3.10), a programmatic verifier, and not by a judge model. A judge in the loop would fold its own preferences into every arm's number, the exact confound chapter 3.6 calibrates against and Part IV formalizes, and a confounded measurement cannot support a clean causal contrast. So I take the exact-metric branch of the menu wherever the oracle can settle the answer, and reserve judged scoring only for the genuinely qualitative RAG items, carrying the 3.6 calibration discipline when I do.

Path evaluation is the piece that matters most for the +tools arm, and it is where this chapter and the practitioner framing meet. The worry in the +tools row above is that the arm posts a large delta for the wrong reason, because the tool handed it the number. Path evaluation ([GADP] chapter 5) is the instrument for catching that. Because the MCP tools return oracle ground truth (the verifiable tool-use of 8.2), I can grade the trajectory, whether the model called the right tool with valid arguments and carried the returned value through, not only the final answer. That splits the +tools delta into "reached the answer" and "used the tool correctly", which is what keeps the arm's claim honest: a high score with a clean tool-use path is tool access working as designed, while a high score with a broken path, the right answer with no tool call behind it, is a leak I want to surface rather than celebrate.

The matched-budget accounting of equations (3.1) and (3.2) is the causal-side answer to a concern [GADP] raises under test-time compute (chapter 5): spending more compute per answer buys accuracy on its own, so an arm that simply draws more decode tokens or makes more tool round-trips can win without reasoning any better. Holding the budget matched is what stops "spent more" from being read as "is better", the same honesty on the intervention side that the exact-metric choice buys on the measurement side. The evaluation patterns are the toolkit for measuring a generative system; the arms are the layer that decides which cause a measurement is allowed to name.
...
For the measurement layer specifically, [GADP] Lakshmanan & Hapke chapters 5 and 6 are the practitioner companion: path and trajectory evaluation (chapter 5) is the working form of the verifiable-tool-use check the +tools arm rests on, and "choosing an evaluation method" with LLM-as-Judge (chapter 6, Pattern 17) is the menu this chapter deliberately picks the exact-metric branch of, for the causal reason that a judged score would confound the very contrast it is meant to measure.
```
