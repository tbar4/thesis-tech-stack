...
### MCP is a protocol, not a framework's tool decorator

Before I write a single tool I want to be clear about what MCP *is*, because the alternative is the default most people reach for and it is the wrong fit for exactly this chapter's needs. The default is to wire each tool into each caller as a framework-specific definition: a hand-written function-calling JSON schema for the OpenAI-style API, or a framework's own decorator such as LangChain's `@tool`. That works, but the tool definition then belongs to the application that declared it. If I want the same conjunction screen available to a second caller, I re-implement the glue against that caller's conventions, and a tool I built for one agent does not port to another without a rewrite. Both [LC] Auffarth & Kuligin (ch. 9) and [GADP] Lakshmanan & Hapke (ch. 7) present the Model Context Protocol as the answer to precisely this fragmentation, and that is why I reach for it rather than a LangChain toolbelt or a loose bag of JSON schemas kept in sync by hand.

MCP is an open *client-server protocol*. A server exposes typed *tools* (and resources) that any MCP-capable *client* can discover and call over a defined *transport* (stdio for a local subprocess, streamable-HTTP for a networked host). The division of labour is the whole point: the model, or the agent driving it, is the client; the thing that provides the tool is the server; the protocol is the contract between them, and neither side needs to know how the other is built. This is the same plan-act-observe tool-use framing [AIE] Huyen ch. 6 and the agentic patterns in [GADP] describe, extending the model's action space beyond text, with one addition the framing alone does not give you: the tool interface is standardized across callers, so "hand the model an action beyond generation" stops being an application-specific integration and becomes a wire format.

That standardization is what earns MCP its place here specifically. Because the protocol decouples the tool provider from the model, one server drives more than one client without duplication: in this chapter the Inspect agentic eval (3.4) and the deployed thin vLLM tool-loop both talk to the identical `sda-tools` server, and neither re-declares the schemas. Just as importantly, the server becomes the single boundary where auth, rate limits, caching, and the 3.9 open-data and redistribution rules all live, which is exactly where the previous subsection argued the space-track boundary has to sit. A standard protocol is what turns "the tool is the boundary" from a one-off arrangement into a reusable architecture: the server is a portable substrate any conforming client can point at. The chapter's server is a concrete instance of that standard, and the verifiable tool-use I build on top (the oracle behind the tool returning ground truth I can grade the model's *process* against) is the layer this chapter adds over the portable substrate, not a property MCP hands me for free.

### FastMCP: the typed signature is the contract

The protocol framing above is what MCP *is*; FastMCP is what makes an instance of it cheap to write. The official `mcp` SDK ships FastMCP, which derives each tool's schema straight from the function's type hints and docstring, so the typed signature `screen_conjunction(tle_a: str, tle_b: str, window_h: int, threshold_km: float) -> dict` *is* the contract the model sees, its name, its arguments, and its return shape all read off the annotations. I write each tool once as an annotated Python function on the server, and that single declaration serves both consumers from the previous subsection, the thin vLLM loop and the Inspect eval, with no second schema to keep in sync by hand. That is the concrete payoff of reaching for the protocol rather than a dictionary of Python callables kept aligned by discipline: a reusable, transport-agnostic tool surface that Part IX's burst work and Part X's assembly can point at without re-deriving anything.
