...
Where the table *lives* follows the storage tier contract from the hardware baseline. LanceDB speaks `s3://` natively, but ANN queries against network-attached spinning disk pay a latency tax on every hop, so the pattern is **build and query on the working tier, sync to MinIO for durability**. The sync has its own discipline, because a crash mid-copy would leave a half-written table as the durable copy: never sync into an existing prefix. Each built table syncs to a fresh, hash-named prefix on MinIO, and a tiny manifest file (the "current index" pointer, written last) names which prefix is live. Rebuilding the working copy after an SSD loss is then a pull of whatever the manifest points at. The same key structure is the model-upgrade path: because a table is named by (corpus hash, embedding model, chunk params), switching embedding models later is not a migration, it is a new table built beside the old one, a retrieval bake-off on the same gold queries, and a one-line manifest flip, with the old instrument still frozen and citable.
```
