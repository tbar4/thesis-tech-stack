...
### Frameworks scaffold the decision; RL learns it

The tool-call loop that unrolls (1.1) is not something I invented for this chapter. It is the same control loop the agent frameworks are built to give you, and naming that overlap is the cleanest way to say what agentic RL actually changes. The dominant pattern is ReAct ([LC]): a prompt template that instructs the model to interleave a reasoning step, a tool action, and an observation, then loop, which is exactly the reason-act-observe rhythm of the $a_t, o_t$ spans in (1.1). LangGraph ([LC]; [AGENTS] ch. 5, ch. 11) makes that loop explicit as a graph, where nodes are steps (call the model, call a tool, route on the result) and edges are control flow the developer draws, so the agent's decision procedure becomes an inspectable state machine rather than an implicit prompt convention. Around that core sit the rest of the production patterns: a short-term scratchpad that carries the running record of tool calls and observations, plus a long-term memory ([LC]; [AGENTS] ch. 14); plan-and-execute, where the agent first drafts a sequence of steps and then runs them ([LC]); multi-agent systems that route work across several specialized agents ([LC]; [AGENTS] ch. 12); and the human-in-the-loop approval gates and guardrails that wrap the whole thing in safety checks ([LC]; [AGENTS] ch. 14). The tools underneath are MCP servers ([AGENTS] ch. 13), which is precisely the 8.2 substrate this book already built.

Here is the one thing every item on that list has in common, and it is the whole point. In each of them the *decision* of when and how to act lives in code and prompts the developer writes. A ReAct prompt tells the model to think-then-act; a LangGraph edge decides whether the next node is another tool call or the final answer; a planner node lays out the steps in advance; a guardrail vetoes a call after the fact. The model is a fixed policy dropped into a control structure that surrounds it, and the competence of the agent is, to a large degree, the competence of that surrounding scaffold. This is a genuine strength for shipping: the control flow is explicit, auditable, and changeable without touching weights, which is exactly what you want for the memory, guardrail, human-in-the-loop, and routing concerns above.

Agentic RL moves one specific decision out of that scaffold and into the policy's weights. It does not replace ReAct or LangGraph; it replaces the *hand-authored answer* to "should I call `screen_conjunction` now, with what arguments, and have I called it enough?" with a learned one. Where the framework encodes that decision as a prompt instruction or a routing edge, the training of (1.3) presses it into $\pi_\theta$ directly, against the verifiable 3.10 reward, so the checkpoint that comes out has internalized when a question is numeric enough to need the oracle, what arguments the call takes, and when it has enough to stop. The slogan is the honest summary: the frameworks *scaffold* the tool-use decision, and RL *learns* it. That is the same move Part V made for the reasoning step, now aimed at the act step of reason-act-observe.

The production synthesis, which matters because this is the product Part, is that you combine the two rather than choose between them. The deployed agent stays a LangGraph or ReAct harness, because I still want the framework to own the things it owns well: the memory and scratchpad, the guardrails and approval gates, the multi-agent routing, the observability of an explicit graph. Its tools are the 8.2 MCP servers. What changes is only the node that decides the tool call: instead of a fixed policy following a prompt convention, that node runs the RL-trained policy from this chapter. Seen this way, the scaffolded framework agent, a fixed policy handed a tool loop, is precisely the strong baseline that 8.3's `+tools` arm measures, a capable model given the tool and a harness that drives it. Agentic RL is the upgrade from that scaffolded decision to a learned one, and the product question of the Lab is exactly whether the learned decision matches or beats the scaffolded one at a lower unnecessary-call rate.
...
For the framework side this chapter contrasts itself against, **[LC] Auffarth & Kuligin** and **[AGENTS] Infante, *AI Agents and Applications*** are the companions: read their ReAct, LangGraph, memory, and multi-agent chapters (and [AGENTS] ch. 13 on MCP servers, ch. 14 on memory and guardrails) as the scaffolded-agent world I am learning to drive from inside. They build the harness by hand; this chapter trains the decision the harness would otherwise script, and in production the two compose.
```
