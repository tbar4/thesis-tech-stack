...
### Building it by hand: the [BRM] companion

Everything in this chapter is a derivation: I have written GRPO as equations and argued about which term does what. If you want to *build* the object rather than derive it, Raschka's **[BRM]** ch. 6-7 assembles exactly this pipeline in a few hundred lines of plain PyTorch with no RL framework, and it is the from-scratch companion I would read alongside these equations. Its ch. 6 opens by motivating verifiable rewards over RLHF (6.1) and then lays out RLVR-with-GRPO through a chef analogy (6.2): a batch of dishes cooked from the same order, each tasted and scored, each judged against the batch average rather than an absolute standard. That is precisely the group baseline of equation (8.1), the mean-subtraction told as intuition instead of $r_i - \bar{r}$.

From there [BRM] ch. 6 walks the same rollout-to-loss pipeline this chapter writes as equation (8.2), one code stage per stage of the math: sampling the group of rollouts (6.5), scoring them to get rewards (6.6), turning rewards into advantages (6.7, the code form of equation 8.1), computing sequence log-probabilities for the importance ratio (6.8), assembling the GRPO loss (6.9), and closing the full function and training loop (6.10-6.11). Reading it makes concrete the thing the derivation states abstractly, that the advantage is a mean-and-std over $G$ scalars and everything else is bookkeeping around the clipped ratio.

The stabilizers this chapter derives arrive in [BRM] ch. 7 as separate, addable pieces, which is the right way to understand them. The clipped policy ratios of ch. 7.4 are the code form of the clipped surrogate in equation (8.2), and the KL loss term of ch. 7.5 is the $-\beta D_{\text{KL}}[\pi_\theta\|\pi_{\text{ref}}]$ penalty with the k3 estimator of equation (8.3); ch. 7.3's advantage-and-entropy tracking is the same instrumentation the gotcha above insists on. [BRM] ch. 7 also builds an explicit `<think>`-token format reward (7.6), which is the shaping term the Part VII lab reuses when it grades not just the answer but whether the model reasoned in the expected structure. The short version: 5.8 tells you *why* each term is there; [BRM] ch. 6-7 shows you the few hundred lines that put it there.
