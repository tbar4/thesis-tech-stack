...
The stack is deliberately boring and standard: Apache Airflow 3 schedules, **dlt** extracts and lands the public feeds (with the `spacetrack` library for the one authenticated source), plain `uv run` modules freeze each load into an immutable snapshot, Pydantic and pandera gate the shapes, Parquet and DuckDB store and query, and DVC plus MLflow pin and log the provenance. Durable bytes live on the archive tier's MinIO object store (the NAS tier from the hardware baseline), and above the frozen snapshots sits a thin serving layer, Delta Lake tables written with `delta-rs` and SQL transforms organized by `dbt-duckdb`, that exists to be queried, not to be cited. It all lives in the CPU/IO `data/` sub-project, which never touches the GPU.
...
The compose file extends the tracking spine: same project, shared network, `data/` mounted into the scheduler so the `uv run` modules and their lock are on the execution path.
